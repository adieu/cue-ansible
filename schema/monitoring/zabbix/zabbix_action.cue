		...
		conditions: {
			...
			suboptions: {
				...
				value: description: [
					...
					"When I(type) is set to C(discovery_status), the choices are C(up), C(down), C(discovered), C(lost).",
					"When I(type) is set to C(discovery_object), the choices are C(host), C(service).",
					"When I(type) is set to C(event_type), the choices are C(item in not supported state), C(item in normal state), C(LLD rule in not supported state), C(LLD rule in normal state), C(trigger in unknown state), C(trigger in normal state).",
					"When I(type) is set to C(trigger_severity), the choices are (case-insensitive) C(not classified), C(information), C(warning), C(average), C(high), C(disaster) irrespective of user-visible names being changed in Zabbix. Defaults to C(not classified) if omitted.",
